...
if($tab==1)
{
	...
	//$minbed = $cmb_bed[0];cmb_bed_min1
	//$maxbed = $cmb_bed[1];
	$minbed1 = $_REQUEST['cmb_bed_min1'];
	$maxbed1 = $_REQUEST['cmb_bed_max1'];
	//$minrentPrice = $cmb_price1[0]*1000;
	//$maxrentPrice = $cmb_price1[1]*1000;
	$minrentPrice = $_REQUEST['priceMin1'];
	$maxrentPrice = $_REQUEST['priceMax1'];
}
if($tab==2)
{
	...
	//$minbed = $cmb_bed[0];
	//$maxbed = $cmb_bed[1];
	$minbed = $_REQUEST['cmb_bed_min'];
	$maxbed = $_REQUEST['cmb_bed_max'];
	//$minsalePrice = $cmb_price2[0]*1000000;
	//$maxsalePrice = $cmb_price2[1]*1000000;
	$minsalePrice = $_REQUEST['priceMin'];
	$maxsalePrice = $_REQUEST['priceMax'];
}
...
WHERE p.is_active = 'Y' AND p.is_sold = 'N' AND p.is_rejected = 'N' AND ";
$qstr = "&tab=$tab";
if($type!="")
{
...
{
	$sqlQry .= "(c.city_name LIKE '%$key%' OR d.name LIKE '%$key%' OR p.prop_name LIKE '%$key%' OR p.prop_desc LIKE '%$key%' OR p.prop_ref_no LIKE '%$key%') AND ";
	$qstr .= "&txtKey=$key";
}
if($tab==1)
{
...
if($minrentPrice!="")
{
	$sqlQry .= "p.annual_rent >= $minrentPrice AND ";
	$qstr .= "&priceMin1=$minrentPrice";
}
if($maxrentPrice!="")
{
	$sqlQry .= "p.annual_rent <= $maxrentPrice AND ";
	$qstr .= "&priceMax1=$maxrentPrice";
}
if($minbed1!="")
{
	$sqlQry .= "b.bed_val >= $minbed1 AND ";
	$qstr .= "&cmb_bed_min1=$minbed1";
}
if($maxbed1!="")
{
	$sqlQry .= "b.bed_val <= $maxbed1 AND ";
	$qstr .= "&cmb_bed_max1=$maxbed1";
}
}
if($tab==1 and $typecat!="")
{
...
if($minsalePrice!="")
{
	$sqlQry .= "p.prop_price >= $minsalePrice AND ";
	$qstr .= "&priceSale=$priceSale";
}
if($maxsalePrice!="")
{
	$sqlQry .= "p.prop_price <= $maxsalePrice AND ";
	$qstr .= "&priceSale=$priceSale";
}
if($minbed!="")
{
	$sqlQry .= "b.bed_val >= $minbed AND ";
	$qstr .= "&cmb_bed_min=$minbed";
}
if($maxbed!="")
{
	$sqlQry .= "b.bed_val <= $maxbed AND ";
	$qstr .= "&cmb_bed_max=$maxbed";
}
}
if($tab==2 and $typecat!="")
{
          ...
          <ul class="ref">
            ...
            <li>
              ...
              <select id="bedroom" name="bedroom" style=" background:#fff;" onChange="document.refine.submit();">
                ...
                <option value="0-4" <?php if($cmb_bedrent=='0-4' || $cmb_bedsale=='0-4') { ?> selected<?php } ?>>0-4</option>
                <option value="4-8" <?php if($cmb_bedrent=='4-8' || $cmb_bedsale=='4-8') { ?> selected<?php } ?>>4-8</option>
                <option value="8-10" <?php if($cmb_bedrent=='8-10' || $cmb_bedsale=='8-10') { ?> selected<?php } ?>>8-10</option>
              </select>
            </li>
            <li>
        ...
        <ul>
          <?php
		   ...
		   foreach($resQry as $rowQry)
		   {
				...
				$sqlImg2 = "SELECT * FROM property_image_master WHERE prop_ref_id = '$propid' and display_order='1'";
				$rowImg2 = $obj->select($sqlImg2);
				$count=count($rowImg2);
				if($count>0)
				{
				$sqlImg = "SELECT * FROM property_image_master WHERE prop_ref_id = '$propid' and display_order='1'";
				$rowImg = $obj->select($sqlImg);
				}
				else
				{
				$sqlImg = "SELECT * FROM property_image_master WHERE prop_ref_id = '$propid' and display_order='1' LIMIT 0,1";
				$rowImg = $obj->select($sqlImg);
				 }
