...
$key1 = get_param('txtKey') ? get_param('txtKey')  : get_session('txtKey') ;
$type = get_param('type') ? get_param('type')  : get_session('type') ;
$community = get_param('community') ? get_param('community')  : get_session('community') ;
$page2 = get_param('page2') ? get_param('page2')  : get_session('page2') ;
$range =get_param('range') ? get_param('range')  : get_session('range') ;
$typecat =get_param('typecat') ? get_param('typecat')  : get_session('typecat');
if($tab==1)
{
	$priceRent = get_param('priceRent') ? get_param('priceRent')  : get_session('priceRent');
	$cmb_price1 = explode('-',$priceRent);
	$cmb_bedrent = get_param('bedroom') ? get_param('bedroom') : (get_param('bedRent') ? get_param('bedRent') : get_session('bedRent'));
	$cmb_bed = explode('-',$cmb_bedrent);
	$minbed = $cmb_bed[0];
	$maxbed = $cmb_bed[1];
...
if($tab==2)
{
	$priceSale = get_param('priceSale') ? get_param('priceSale')  : get_session('priceSale');
	$cmb_price2 = explode("-",$priceSale);
	$cmb_bedsale = get_param('bedroom') ? get_param('bedroom') : (get_param('bedSale') ? get_param('bedSale') : get_session('bedSale'));
	$cmb_bed = explode('-',$cmb_bedsale);
	$minbed = $cmb_bed[0];
	$maxbed = $cmb_bed[1];
...
echo $sqlQry = "SELECT p.* FROM property_master p LEFT JOIN property_type_master t ON p.type_ref_id=t.typeid 
...
if($type!="" && $type!='a' )
{
	$sqlQry .= " AND p.type_ref_id = '$type' ";
	set_session('type',$type);
}
if($typecat!="")
{
...
{
	$sqlQry .= " AND p.development = '$community'";
	set_session('community',$community);
}
if($key1!="Your Keyword here" && !empty($key1))
{
	$sqlQry .= " AND (c.city_name LIKE '%$key1%' OR d.name LIKE '%$key1%' OR p.prop_name LIKE '%$key1%' )";
	set_session('txtKey',$key);
}
if($tab==1)
{
...
{
	$sqlQry .= " AND (p.annual_rent > $minrentPrice AND p.annual_rent < $maxrentPrice)";
	set_session('priceRent',$priceRent);
	$sqlQry .= " AND (b.bed_val >= $minbed AND b.bed_val <= $maxbed)";
	set_session('bedRent',$cmb_bedrent);
}
if($tab==1 and $typecat!="")
{
	if($minbed!="")
...
{
	$sqlQry .= " AND (p.prop_price > $minsalePrice AND p.prop_price < $maxsalePrice)";
	set_session('priceSale',$priceSale);
	$sqlQry .= " AND (b.bed_val >= $minbed AND b.bed_val <= $maxbed)";
	set_session('bedSale',$cmb_bedsale);
}
if($tab==2 and $typecat!="")
{
	if($minbed!="")
