...
function menuNavigation($menu=NULL,$lang='en',$slug,$parentSlug,$currpage,$type,$cont)
{
...
//			}
			...
			else if($type==$val['name'])
			{
			$aclass = "class='active'";
			//echo 4;
			}
			else if($cont=='other-services' and $val['name']=='Services')
			{
			$aclass = "class='active'";
			//echo 40;
			}
			else if($cont=='valuation' and $val['name']=='Services')
			{
			$aclass = "class='active'";
			//echo 40;
			}
			else if($currpage == $val['link'])
			{
			$aclass = "class='active'";
			//echo 6;
			}
			
			else if($slugStr[1]=='contact-us' and $cont == 'contact-us')
			{
			$aclass = "class='active'";
			//echo 0;
			}
			...
			{
				//********/
				$has_sublevel = $val['has_sublevel'];
				if($has_sublevel == 'Y')
				{
					$getSubData  = $obj->select("SELECT * FROM content WHERE is_active = 'Y' AND parentid = '{$val['link_id']}'");
					if(count($getSubData) > 0)
					{
						$menuNavStr .= '<ul style="'.((count($getParentData) == $key+1) ? (($lang=='ar') ? 'left:-77px;' : '' ) : 'left:0px;').'">';
						foreach($getSubData as $key1 => $val1)
						...
						if($val['id']==31)
						{
								$link_url =$val1['page_url'] ? strip($val1['page_url']) : 'top-locations.php?content='.$val1['slug'];
						}
						else
						{
								$link_url =$val1['page_url'] ? strip($val1['page_url']) : 'content.php?content='.$val1['slug'];
								}
							$menuNavStr .= '
							  <li '.(($key1==0) ? 'class="droupmenu_top'.((!$getParentData[$key+1]) ? '_last' : '').'"' : ((count($getSubData) == $key1+1) ? 'class="droupmenu_bottom"' : '') ).((!$getSubData[$key1+1]) ? ' style="border:none;"' : '').'>
							  <a href="'.$link_url.'" style="'.((!$getSubData[$key1+1] && $lang == 'ar') ? 'border-bottom:none;' : '').((!$getParentData[$key+1]) ? '' : '').'" target="'.stripslashes($val1['target']).'">'.(stripslashes(($lang=='ar') ? ($val1['pagename_arb'] ? $val1['pagename_arb'] : $val1['pagename']) : $val1['pagename'])).'</a></li>';
						}
						$menuNavStr .= '</ul>';
					}
				}
...
function genRefNo ($tmpoutVal = 0,$propfor) {  //echo '<br>';echo $tmpoutVal ;echo '<br>';
	...
	if($propfor=='sales')
	{
	$fldrefno ='DPS-'.$fldstrno;
	}
	if($propfor=='rent')
	{
	$fldrefno ='DPR-'.$fldstrno;
	}
	...
	if(checkExist($fldrefno)) {
		...
		$fldrefno = genRefNo ($tmpoutVal,$propfor);
...
function genRefNo2 ($tmpoutVal = 0) {  //echo '<br>';echo $tmpoutVal ;echo '<br>';



	$brokquery  = "select refno,added_on from booking ORDER BY id DESC LIMIT 1 ";
	...
	$fldrefno ='BOOK-'.$fldstrno;



	if(checkExist2($fldrefno)) {
...
function checkExist2($fldrefno) {



	$brokquery  = "select refno FROM booking WHERE refno ='$fldrefno' ";
